  ...
  <article>
    <div class="mx-auto max-w-3xl px-6">
      <div class="py-8 sm:py-20 border-b border-gray-300">
        <header class="mb-8">
          <h2 class="text-3xl sm:text-4xl leading-tight font-sans mb-1 sm:mb-2 px-2 sm:px-4 md:px-10">
            <g-link :to="`${post.path}/`" class="text-black font-bold">{{ post.title }}</g-link>
          </h2>
          <p class="text-gray-700 leading-normal text-sm sm:text-base px-2 sm:px-4 md:px-10">
            <time :datetime="post.datetime" class="text-gray-700">{{
              formatPublishDate(post.datetime) }}
            </time>
            <span> · </span>
            <span v-if="post.author">by <g-link :to="`${post.author.path}/`"
            ...
            <span v-if="post.tags && post.tags.length > 0"> in <g-link :to="`${post.tags[0].path}/`"
                                                                       class="text-gray-700 capitalize border-b border-transparent hover:border-gray-400 transition-colors duration-300">{{ titleCase(post.tags[0].title) }}</g-link></span>
